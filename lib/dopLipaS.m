...
%           thc1 - thc2 provides an estimate of swell direction accuracy
%
%
%% Uses
...
%  Sci 15(4):843–853
%
%  Lipa, B. J., D. E. Barrick, and J. W. Maresca Jr. "HF radar measurements
%  of long ocean waves." Journal of Geophysical Research: Oceans 86.C5
%  (1981): 4089-4102.
%
%% Copyright 
...
if isnan(f_1) || isnan(f_2) || isnan(f_3) || isnan(f_4)
    ...
    for i = 1:length(ths1)
        for j = 1:length(ks1)
            ...
            k1 = ks1(j);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [~,~,fc1] = gamma_mm_depth(-1,m2_12,k0,k1,th1,d); %  m1 = -1;  % right beam, PXY
            [~,~,fc2] = gamma_mm_depth(+1,m2_12,k0,k1,th1,d); %  m1 = +1;
            [~,~,fc3] = gamma_mm_depth(-1,m2_34,k0,k1,th1+beamdiff,d); % left beam, PXY1
            [~,~,fc4] = gamma_mm_depth(+1,m2_34,k0,k1,th1+beamdiff,d);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % theoretical f
            F1 = fc1 - m2_12*fbragg;
            F2 = fc2 - m2_12*fbragg;
