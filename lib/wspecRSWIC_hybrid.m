...
%function [Sfw,Sfs,Sf,th] = wspecRWIC(f,Rw,Sf_wind,Sf_swell,fc,thw,ths,jswaproll); % this was used
%
...
if Ratio > RatioLim && max(Sfs) > 0 % Swell dominated conditions for deep waters (use hybrid)
    ...
    j = j - 1;
    hybrid = 1; % use hybrid
else % just wind inversion, force inversion to zero below fc
    [~,j] = min(abs(f-(fc)));
    j = j - 1;
end
...
%         th(1:j) = ths;             % swell direction from Lipa (unused here)
    end
else % wind inversion
